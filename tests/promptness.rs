...
use tiny_http::{Response, Server, http};
    ...
    fn assert_requests_parsed_promptly(
        ...
        spawn(move || {
            for _ in 0..req_cnt {
                ...
                spawn(move || {
                    req.respond(Response::empty(http::StatusCode::OK).with_data(resp_body, Some(resp_body.len)))
                });
            }
...
mod prompt_responses {
    ...
    fn assert_responds_promptly(
        ...
        spawn(move || loop {
            ...
            req.respond(Response::empty(http::StatusCode::BAD_REQUEST)).unwrap();
        });
